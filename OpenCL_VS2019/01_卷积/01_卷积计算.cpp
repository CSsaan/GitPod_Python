...
int main()
{
	...
	cout << "	检测到的Platform数量: " << num_platforms  << ",	platform_id: " << platform_id << endl;
	if (CL_SUCCESS != ret)
	{
	...
	cout << "	检测到的Device数量: " << num_devices << ",	device_id: " << device_id << endl;
	if (CL_SUCCESS != ret)
	{
	...
	/*******************************************************************************/
	/** step3: 使用platform_id、device_id创建Context */
	/*******************************************************************************/
	...
	/*******************************************************************************/
	/** step5: 创建Memory Object */
	/*******************************************************************************/
	cout << "[5.]开始‘创建Memory Object’" << endl;
	cl_mem mem_obj1 = NULL;
	cl_mem mem_obj2 = NULL;
	cl_mem mem_objResult = NULL;
	const int ARRAY_SIZE = 8 * 8;
	const int MASK_SIZE = 3 * 3;
	const int OUT_SIZE = 6 * 6;
	const int BUF_SIZE = ARRAY_SIZE * sizeof(cl_uint);
	const int BUF2_SIZE = MASK_SIZE * sizeof(cl_uint);
	const int BUF3_SIZE = OUT_SIZE * sizeof(cl_uint);

	cl_uint inputSignal[8][8] =
	{
		{3, 1, 1, 4, 8, 2, 1, 3 },
		{3, 1, 1, 4, 8, 2, 1, 3 },
		{3, 1, 1, 4, 8, 2, 1, 3 },
	...
	mem_obj1 = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, BUF_SIZE, static_cast<void*> (inputSignal), &ret);
	mem_obj2 = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, BUF2_SIZE, static_cast<void*>(mask), &ret2);
	mem_objResult = clCreateBuffer(context, CL_MEM_WRITE_ONLY, BUF3_SIZE, NULL, &retResult);
	// 检查错误
	...
	/* kernel代码 */
	// 法一：从文件读取kernel代码
	string strkernelSource = loadCodeFromFile("../resouces/source_01.cl");
	...
	size_t global_work_size[] = { 6 * 6, 1, 1 };
	//size_t global_work_size[1] = { 6 * 6 };
	...
	int* device_buffer = (int*)clEnqueueMapBuffer(command_queue, mem_objResult, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, BUF3_SIZE, 0, NULL, NULL, &ret);
