...
#define KERNEL_DIR "../resouces/source_02.cl"
#define KERNEL_FUNCTION_NAME "kernel_rgb2gray"
...
int main()
{
	cl_int ret, ret2, ret3, retResult;
	/*******************************************************************************/
	...
	const int ARRAY_GRAY_SIZE = img_h * img_w;
	...
	const int BUF4_GRAYImage_SIZE = ARRAY_GRAY_SIZE * sizeof(uchar);
	...
	//法二：创建vector
	/* 申请buffer内存&初始化 */
	mem_obj1 = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, BUF1_RGBImage_SIZE, srcImage.data, &ret);
	...
	mem_objResult = clCreateBuffer(context, CL_MEM_WRITE_ONLY, BUF4_GRAYImage_SIZE, NULL, &retResult);
	// 检查错误
	if ((CL_SUCCESS != ret) || (CL_SUCCESS != ret2) || (CL_SUCCESS != ret3) || (CL_SUCCESS != retResult) || (NULL == mem_obj1) || (NULL == mem_obj2) || (NULL == mem_objResult))
	{
	...
	size_t global_work_size[2] = { (size_t)((img_w + 32 - 1) / 32) * 32,
								   (size_t)((img_h + 4 - 1) / 4) * 4 };
	...
	//【法二】：直接读取图片缓冲buffer
	Mat grayImage(img_h, img_w, CV_8UC1, Scalar(0, 0, 0));
	ret = clEnqueueReadBuffer(command_queue, mem_objResult, CL_TRUE, 0, sizeof(uchar) * img_h * img_w, grayImage.data, 0, NULL, NULL);
	if (CL_SUCCESS != ret)
	{
		cout << "读取结果失败！" << endl;
		return -1;
